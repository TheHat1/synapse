...
func _on_line_edit_text_submitted(new_text: String) -> void:
	$Control2/Control/LineEdit.keep_editing_on_text_submit = true
	if !new_text.is_valid_float():
		ErrorMessage.show_error("Invalid value")
		$Control2/Control/LineEdit.text = old_text
		return
	...
	drop = new_text.to_float()
	old_text = new_text
	$Control2/Control/LineEdit.text = str(drop)
	$Control2/Control/LineEdit.keep_editing_on_text_submit = false

func _on_value_changed(value, value2):
	vcc = value
	charge_resistor = value2
	if enabled:
		emit_signal("value_changed_resistor", value - drop, value2)
	else:
...
func _on_gate_status_change(status: bool):
	...
	if status:
		...
		emit_signal("value_changed_resistor", vcc - drop, charge_resistor)
	else :
...
func _input(event):
	if event is InputEventKey and event.keycode == KEY_DELETE and event.pressed and is_selected():
		get_parent().get_parent().get_parent().get_node("MainMenu").bucks_deleted += 1
		queue_free()
